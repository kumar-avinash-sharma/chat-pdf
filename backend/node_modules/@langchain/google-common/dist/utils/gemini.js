...
function getGeminiAPI(config) {
	...
	*/
	function multimodalContentBlockToPart(block, defaultMimeType) {
		...
		if ("url" in block && block.url !== void 0) {
			...
			return { fileData: {
				mimeType: block.mimeType || inferMimeTypeFromUrl(block.url) || defaultMimeType,
				fileUri: block.url
			} };
		}
		if ("fileId" in block && block.fileId !== void 0) throw new Error("ContentBlock.Multimodal fileId is not supported by Google Gemini. Use a URL (e.g. gs:// URI) or base64 data instead.");
	...
	async function messageContentComplexToPart(content) {
		switch (content.type) {
			...
			case "input_audio":
				...
				break;
			case "image": return multimodalContentBlockToPart(content, "image/png");
			case "video": return multimodalContentBlockToPart(content, "video/mp4");
			case "audio": return multimodalContentBlockToPart(content, "audio/mpeg");
			case "file": return multimodalContentBlockToPart(content, "application/octet-stream");
			case "text-plain":
				if ("text" in content && typeof content.text === "string") return { text: content.text };
				return multimodalContentBlockToPart(content, "text/plain");
